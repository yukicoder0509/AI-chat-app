...
import type { OpenAIRequestBody, OpenAIResponse, OpenAIStreamResponse, OpenAIErrorResponse } from '../../types/openai'
...
export class OpenAIClient {
   ...
   */
  async createChatCompletion(body: Omit<OpenAIRequestBody, 'stream'>): Promise<OpenAIResponse> {
    const requestBody: OpenAIRequestBody = {
      ...body,
      stream: false,
    }

    const response = await this.makeRequest<OpenAIResponse>('/v1/chat/completions', requestBody)
    return response
  }
   ...
   */
  async *streamChatCompletion(body: Omit<OpenAIRequestBody, 'stream'>): AsyncIterable<OpenAIStreamResponse> {
    const requestBody: OpenAIRequestBody = {
      ...body,
      stream: true,
    }
...
export function getOpenAIClient(config?: OpenAIClientConfig): OpenAIClient {
  if (!clientInstance) {
    if (!config) {
      throw new Error('OpenAI client configuration is required for initialization')
    }
